...
generate_contingency_plot <- function(variable_name) {
  ...
  # Create the plot
  plot <- ggplot(contingency_table2, aes(x = factor(!!sym(variable_name)), y = Prop, fill = CategorizedResponse)) +
...
for (i in seq_along(results)) {
  ...
  write.csv(results[[i]]$contingency_table, csv_file, row.names = FALSE)
...
p <- filtered_race_df %>%
  ...
  facet_wrap(~Unique_Value, ncol=4) +
  xlim(0, 1.0) + 
  ylim(0, 1.0) +
...
p
ggsave(paste0(base_dir, tables_dir, "race_intersection_TPR_x_FPR.png"), plot = p, width = 10, height = 6)
