...
rates_data <- df_cleaned %>%
  group_by(PromptID, Age, Model, CategorizedResponse, .groups = "drop") %>%
  summarise(count = n()) %>%
...
melted_data <- melt(rates_data, id.vars = c("PromptID", "Model", "Age"), variable.name = "RateType", value.name = "Rate")

# Calculate the fractions of refusals and blocks for each race and each Model
fraction_data <- aggregate(Rate ~ PromptID + Model + Age + RateType, melted_data, sum) %>% filter(RateType != 'Diagnosis')
...
plot <- ggplot(fraction_data, aes(x = Age, y = Rate, fill = Age, shape = Model)) +
  geom_point(size = 4, position = position_dodge(width = 1.0), aes(color = Age), stroke = 1.0, alpha = 1) +
  scale_fill_brewer(palette = "Set1") +
  #scale_shape_manual(values = c(0, 15, 1, 16, 2, 17, 5, 18)) +  # Manual shape mapping
  ...
  facet_grid(RateType ~ PromptID, scales = "fixed") +  # Adjusted facet_grid layout
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    legend.title = element_text(size = 12),
    legend.text = element_text(size = 10),
    legend.position = "bottom",
    panel.background = element_rect(fill = "white"),  # Set white background for plot panel
    plot.background = element_rect(fill = "white")     # Set white background for entire plot
  ) +
  theme_minimal() +
...
# Display the plot
ggsave(paste0(base_dir, figures_dir, "facet_plot_with_consistent_jitter.png"), plot,  width = 13, height = 3 , units = "in", dpi = 300,  bg = "white")  # Set the b
...
for (i in seq_along(dfs)) {
  ...
  if (file.exists(outfile_path)) {
    ...
    demographic_groups <- c("Overall", "Age")
...
p <- ggplot(filtered_demo_df, aes(x = Unique_Value, y = Balanced.Accuracy_mean, color = Model, shape = PromptID)) +
  geom_errorbar(aes(ymin = Balanced.Accuracy_lower, ymax = Balanced.Accuracy_upper), alpha = 0.3) +
  geom_point(size = 3) +
...
ggsave(paste0(base_dir, figures_dir, "age_overlap_intersection_cp_balanced_acc.png"), plot = p, width = 10, height = 4 , units = "in", dpi = 300, bg = "white")
 print(p)
```
...
p
ggsave(paste0(base_dir, figures_dir, "race_intersection_TPR_x_FPR.png"), plot = p, width = 10, height = 6)
